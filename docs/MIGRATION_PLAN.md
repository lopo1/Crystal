...
| 组队 (Group) | 🟨 | GroupManager(建队/邀请/接受/离队/解散) + Group 协议端到端；公会/交易/邮件/市场未做 |
| 地图: 障碍/传送门/刷怪点 | 🟨 | 已接入真实 .map（V100 + wemade2010 双格式）碰撞；多图注册表 + /map 传送 + 走格传送门 + **每张图独立怪物/NPC/掉落（map_index 隔离、广播/AI/拾取按图作用域）** 已打通（刷怪点配置/怪物数据化未做） |
